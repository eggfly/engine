...
bool MessageLoopTaskQueues::Merge(TaskQueueId owner, TaskQueueId subsumed) {
  FML_LOG(ERROR) << "---- eggfly ---- real Merge() this=" << this << ", owner=" << owner
                 << ", subsumed=" << subsumed
                 << ", owner_of.size=" << queue_entries_.at(owner)->owner_of.size();
  if (owner == subsumed) {
    return true;
  }
  std::lock_guard guard(queue_mutex_);
...
bool MessageLoopTaskQueues::Unmerge(TaskQueueId owner, TaskQueueId subsumed) {
  ...
  FML_LOG(ERROR) << "---- eggfly ---- real Unmerge() this=" << this << ", owner=" << owner
                 << ", subsumed=" << subsumed
                 << ", owner_of.size=" << queue_entries_.at(owner)->owner_of.size();
...
//  FML_LOG(ERROR) << "---- eggfly ---- this=" << this << ", owner=" << owner
//                 << ", subsumed=" << subsumed
//                 << ", owner_of=" << queue_entries_.at(owner)->owner_of;
  return owns;
}
